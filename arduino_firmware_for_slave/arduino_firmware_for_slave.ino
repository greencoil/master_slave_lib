...
int find_header(byte packet[]) {
  int target_index = virtual_buffer_head_decrement(buff_head);
  for(int i=3;i>=0;i--){
    if(!(packet[target_index] == HEADER[i]))return false;
    target_index = virtual_buffer_head_decrement(target_index);
    // Serial1.print("Cmp:");
...
int8_t virtual_buffer_head_decrement(int head){
    int new_head = head-1;
